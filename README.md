...
**重要：请仅安装你绝对信任的 PWA 应用！**

本容器为了提供完整的功能，对 PWA 应用进行了以下权限开放：

| 权限 | 说明 | 风险 |
|------|------|------|
| **跨域请求** | 禁用 CORS，可访问任意 API | 恶意 PWA 可窃取其他网站数据 |
| **文件系统** | OPFS 劫持到原生文件系统 | 恶意 PWA 可能访问你的文件 |
| **Cookie 管理** | 持久化存储 Cookie | 同一域名下 Cookie 共享 |
| **代理设置** | 可配置 HTTP/SOCKS5 代理 | 恶意 PWA 可劫持网络流量 |

**虽然不同 PWA 之间的 Cookie 是隔离的**，但如果你安装了恶意 PWA，它仍然可能：
- 以你的名义发送请求
- 访问你授权的资源
- 利用你的网络环境攻击其他服务

**建议**：
- ✅ 只安装知名、可信的 PWA（如 Twitter、Telegram、WhatsApp 等官方 PWA）
- ❌ 不要安装来源不明的 PWA
- ❌ 不要用此容器登录敏感账户（如银行、支付等）
- ⚠️ 定期清除不用的 PWA 数据
